<?php
	...
	class ComentariosModel extends DefaultModel{
		...
		public function getComentariosByProduto($id){
			...
			$query = $this->query()->select(["comentarios.id_comentario, comentarios.mensagem, usuarios.usuario, DATE_FORMAT(comentarios.data_comentario, '%k:%i - %d/%m/%Y') AS data"])->innerJoin(["usuarios" => "usuarios.id_usuario = comentarios.id_usuario"])->where(["comentarios.id_produto =" => $id])->all();
